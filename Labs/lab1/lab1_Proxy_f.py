...
def file_cache(webServer, resource, port, message):
    file_to_use = cache_directory + resource.replace("/", ".")
    fileExist = False
    outputdata = None
    try:
        with open(file_to_use, "rb") as fin:
        ...
        if fileExist == False:
            ...
            buffer = []
            try:
                c.sendall(bytes(message, encoding='ascii'))
                while True:
                    buff = c.recv(2048)
                    if len(buff) > 0:
                        buffer.append(buff)
                ...
                outputdata = b''.join(buffer)
                with open(file_to_use, "wb") as cacheFile:
                    cacheFile.write(outputdata)
                c.close()
        else:
            print("File does not exist, replying with 404")
            outputdata = (bytearray("HTTP/1.1 404 Not Found \n", "ascii"))
    finally:
        return outputdata
